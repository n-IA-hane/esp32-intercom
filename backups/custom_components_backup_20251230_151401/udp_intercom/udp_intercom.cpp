...
void UDPIntercom::dump_config() {
  ...
  if (this->i2s_mode_ == I2S_MODE_DUAL) {
    const char* ch_str = "left";
    if (this->mic_channel_ == MIC_CHANNEL_RIGHT) ch_str = "right";
    else if (this->mic_channel_ == MIC_CHANNEL_STEREO) ch_str = "stereo";
    ESP_LOGCONFIG(TAG, "  Mic Config: %d-bit, channel=%s, gain=%dx",
                  this->mic_bits_per_sample_, ch_str, this->mic_gain_);
  }
}
